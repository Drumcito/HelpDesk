...
// ✅ AHORA primero el tiempo (para usarlo en :now)
...
function resolveAvailability(array $a, DateTime $now): array {
    ...
    $dayN = (int)$now->format('N');
    if ($dayN === 7) return ['status' => 'FUERA_DE_HORARIO'];
    ...
    if ($dayN >= 1 && $dayN <= 5) {
        ...
        if (!satWorksToday($pattern, $now)) return ['status' => 'FUERA_DE_HORARIO'];
        $start = '08:00:00';
        $end   = '14:00:00';
    }
