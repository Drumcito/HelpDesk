...
/* alerts */
$alerts = [];
if (isset($_GET['created'])) $alerts[] = ['type'=>'success','icon'=>'capsulin_add.png','text'=>'PRIORIDAD REGISTRADA'];
if (isset($_GET['updated'])) $alerts[] = ['type'=>'info','icon'=>'capsulin_update.png','text'=>'PRIORIDAD ACTUALIZADA'];
if (isset($_GET['toggled'])) $alerts[] = ['type'=>'info','icon'=>'capsulin_update.png','text'=>'PRIORIDAD ACTUALIZADA'];
if (isset($_GET['error']))   $alerts[] = ['type'=>'danger','icon'=>'capsulin_delete.png','text'=>'OCURRIÓ UN ERROR'];

/* fetch */
$st = $pdo->prepare("SELECT id, code, label, sla_hours, active, sort_order, created_at
...
<main class="user-main sa-panel">
  ...
  <header class="panel-top">
    <div class="panel-top-left">
      ...
      <p class="panel-subtitle">Prioridades (Super Admin)</p>
    </div>
  </header>
  ...
  <section class="panel-card">
    <div class="panel-card-head">
      ...
      <div class="directory-actions" style="gap:10px;">
        ...
        <button type="button" class="action-btn action-delete" title="Activar/Desactivar" onclick="togglePriority()">
    ...
    <div class="panel-table-wrap" style="margin-top:12px;">
      <table class="panel-table" id="prioTable">
        <thead>
          <tr>
            <th>ID</th>
            <th>Code (BD)</th>
        ...
        <tbody>
        <?php if(!$rows): ?>
          <tr><td colspan="7" class="panel-empty">No hay prioridades.</td></tr>
        <?php else: foreach($rows as $r): ?>
          <tr class="catalog-row"
              data-id="<?= (int)$r['id'] ?>"
              data-code="<?= h($r['code']) ?>"
              data-label="<?= h($r['label']) ?>"
              data-sla="<?= (int)$r['sla_hours'] ?>"
  ...
  <div class="modal-backdrop" id="modal-create">
    <div class="modal-card">
      ...
      <p class="modal-description">El <b>code</b> debe coincidir con lo que guardas en tickets (baja/media/alta).</p>

      <form class="modal-form" method="POST" action="/HelpDesk_EQF/modules/dashboard/sa/priority_catalog_actions.php">
        <input type="hidden" name="action" value="create">

        <div class="modal-grid">
          <div class="form-group">
            <label>Code (BD)</label>
            <input type="text" name="code" placeholder="media" required>
          </div>
          <div class="form-group">
            <label>Etiqueta</label>
            <input type="text" name="label" placeholder="Media" required>
          </div>
          <div class="form-group">
            <label>SLA (horas)</label>
            <input type="number" name="sla_hours" value="8" min="0" required>
          </div>
          <div class="form-group">
            <label>Orden</label>
  ...
  <div class="modal-backdrop" id="modal-edit">
    <div class="modal-card">
      ...
      <p class="modal-description">Edita la prioridad seleccionada.</p>
      ...
      <form class="modal-form" method="POST" action="/HelpDesk_EQF/modules/dashboard/sa/priority_catalog_actions.php">
        ...
        <div class="modal-grid">
          ...
          <div class="form-group">
            <label>SLA (horas)</label>
            <input type="number" name="sla_hours" id="edit_sla" min="0" required>
          </div>
          <div class="form-group">
            <label>Orden</label>
  ...
  <!-- FORM TOGGLE (hidden) -->
  <form id="toggleForm" method="POST" action="/HelpDesk_EQF/modules/dashboard/sa/priority_catalog_actions.php" style="display:none;">
...
function openEditModal(){
  ...
  document.getElementById('edit_sla').value = selected.dataset.sla;
...
function togglePriority(){
  ...
  const msg = isActive
    ? '¿Desactivar esta prioridad? (no se borra)'
    : '¿Activar esta prioridad?';
