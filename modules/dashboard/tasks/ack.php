<?php
// /HelpDesk_EQF/modules/dashboard/tasks/ack.php
session_start();
...
if (!$row || $row['status'] !== 'ASIGNADA') {
    header('Location: /HelpDesk_EQF/modules/dashboard/tasks/analyst.php'); exit;
}

$pdo->prepare("UPDATE tasks SET status='EN_PROCESO', ack_at=NOW() WHERE id=?")->execute([$taskId]);

logTaskEvent($pdo, $taskId, $analystId, 'ACKED', 'Analista marcó enterado', ['status'=>'ASIGNADA'], ['status'=>'EN_PROCESO']);
